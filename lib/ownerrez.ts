...
export async function syncOwnerRezForHost(hostId: string): Promise<{
  ...
  try {
    ...
    for (const b of bookings) {
      ...
      // If such a booking was previously synced as an active event, delete that
      // stale event now (this is the targeted cleanup that removes duplicates
      // without ever touching a booking OwnerRez still reports as active).
      const isBlock = pick<boolean>(b, ["is_block", "isBlock"]) === true;
      ...
      if (isBlock || canceledAt || orStatus.includes("cancel")) {
        const staleUid = `ownerrez-${String(pick(b, ["id", "bookingId"]))}`;
        await db.from("calendar_events").delete().eq("host_id", hostId).eq("event_uid", staleUid);
        continue;
      }
